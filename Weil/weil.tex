...
groups; furthermore, if $B_{0}(G)$ is the normalizer of $\mathbf{A}(G)$
...
$B_0(G)/T$ is isomorphic to $B_0(G)$, 	
...
We will see that the operators of $B_0(G)$ have the same property.
...
on $G$; such that $P\cdot D\Phi$ or simply $G$ 
...
of continuous functions on $G$, which upported in $H$, and constant on cosets 
of $H'$, 	
by restriction to H and passage to the quotient $H/H'$, 
it  belongs to $S(H/H')$. 	
...
We intend to see that any $s\in B_0(G)$ induce an automorphism of $S(G)$;
We only have to show that $s\in B_0(G)$ induce a continuous map 
from $S(G)$ to itself, we will follow step by step of the proof of 
the Theorem~\ref{th:1}.  	
We will write again $t$ instead of $s^{-1}$, $p^t$ 	
instead of $s^{-l}P$ for $P\in L^2(G)$, 	
and we will prove for the operator $p\to  pt$.
...
$F(x, y) -> F(yx,-x)$, which obviously an automorphism of $S(G\times G)$,
...
This is not difficulty to varify the  expressing $f$ on the  classes 
...
it is both necessary and sufficient,
 as we know (see [1], $n^o$~9, p. 60) that the group $H$,
...
Replacing $H$ by the group generated by $H$ and $a^*$,
we can make that $a^*$ in $H$;
since, $f$ induces the constant $1$ on $H'$ and 
and is constant in each class following $H'$. 
After what has been demonstrated in the case $G$ is a  elementary group,
