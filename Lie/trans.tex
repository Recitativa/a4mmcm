...
\subsection{$U(r,s)\times U(m)$ embedding}
...
\subsection{$\fuu_\bC(m) = \agl_m$ action}
...
\subsection{explicit $U(r,s) \sim U(m)$ duality}
...
\subsection{Theta corresponedce for $\Sp(2n,\bR)\sim O(m)$}
Note that 
\[
...
$\tDelta_s$ has weight
\[
(0, \cdots, 0, \underbrace{1,\cdots, 1}_{2l+1-s=m-s})
