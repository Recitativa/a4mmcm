...
\]
as $O(2m)$ module. 
\item Similary, $\left(\Gamma_{\fgg,K}^{\fgg,G}\right)^j\theta(D_l) = (\lambda; +1)$ for $j = $.
\end{enumerate}
\end{lemma}
\proof
\begin{enumerate}[a)]
...
=& \left(\sigma \circ \Gamma_{\fgg,K_0}^{\fgg,G})^j
...
\end{split}
\]
Apply last functor for $j=q$, we get the result. 
\end{enumerate}
