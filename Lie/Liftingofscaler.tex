...
and $\rho(\tau)\to \Omega_{\rho(\tau)}(\sigma)\cong \sigma$. $S_i$ are defined similarly.
...
\begin{cor}
  ...
  Suppose 
  \begin{enumerate}[(1)]
  \item $\sigma_i$ is a character of $G_i'$, where $i=1,2$. 
  \item  Let $\cjj_1 = \Ker(\sigma|_{\cuu(\fgg')}) \subset \cuu{\fgg'}$.
    $\cjj_1 = \cjj_2$.
  \item $\tau_1$ is a $K_1$-module such that
    $\dim \Hom_{H_1}(\rho(\sigma_1),\tau_1) = 1$.
  \item $\tau_2$ is a $K_2$-module occure in the
    image of map $\Gamma^i \rho(\sigma_1) \to \Gamma^i \tau$ 
    such that 
    $\dim \Hom_{H_2}(\rho(\sigma_2),\tau_2) =1$.
  \end{enumerate}
...
with $\cuu(\fgg)^{\ckK}$ action on $\mu$ component via $c_{\rho(\sigma),\tau}$.
By Theorem~\ref{thm:ugkcor}, for any $x\in \cuu(\fgg)^{\ckK}$, 
we can find $z\in Z(\fkk)$ such that $c_{\rho(\sigma),\tau}(x) = \chi_\tau(z)$.
On the other hand, $x$ act on $\mu$ isotropic space of $\rho(\csigma)$ 
is by $c_{\rho(\csigma),\mu}(x) = \chi_\mu(z)$.
Since the derived functor preserve the infinitesimal character, 
\[
c_{\rho(\sigma),\tau}(x) = \chi_\tau(z) = \chi_\mu(z)=c_{\rho(\csigma),\mu}(x).
\]
