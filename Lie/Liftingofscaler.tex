...
Let $\mu$ be a irreducible representation of $\ckK$ and $L(\mu)$ be the represntation of theta lifing of $\mu$ to $\ckM'$.

Suppose that $\sigma$ is a $G'$ quotient of $L(\mu)$.
...
\proof
Fix $\gg$,$\gg'$, $\kk$ and $\mm$ in $\sp$. Fix a real from $\sp_0$,
such that $\gg\cap \sp_0 = \gg_0$. There is a inner automorphism $\iota(X) = cXc^{-1}$ of $\Sp$ such
that $\iota$ presever all complex algebra, but $\gg \cap \iota \sp_0=\check{\gg}_0$.
\qed
