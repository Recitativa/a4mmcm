...
$\Theta_{\ckG}(\sigma)$ with same $\ckK$ type $\mu$.


Let $\mu$ be a irreducible representation of $K$ and $L(\mu)$ be the
represntation of theta lifing of $\mu$ to $M'$.  First, I assume the
occurence condition:
\begin{enumerate}[(i)]
\item $\sigma$ is a $G'$ quotient of $L(\mu)$;
\item $\sigma$ occur in both of $(G_i,G')$ ($(i=1,2)$);
\item 
\end{enumerate}
...
The we can pick a symplectic basis $\Set{e_j,f_j}$ of $W_0$.  
There is a unique (upto scaler) $0\neq v\in S$ such that $(e_j-if_j)\cdot v = 0$. 
