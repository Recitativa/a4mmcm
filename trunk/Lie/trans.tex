...
\subsection{$(\O(p,q),\SL(2,\bR)$ dual pair}
...
with $\O(n,\bC)$ act on left, $SL(2,\bC)$ act on right. 
Give symplectic form on $W$ by tensoring the form 
of $\O(n,\bC)$ and $\SL(2,\bC)$, so we have complex symplectic group act on $W$.
$\O(n,\bC)$ and $\SL(2,\bC)$ are complex reductive dual pair 
Choose real subgroup stablize $W_{p,q}$, we have $\O(p,q)$ and $\SL(2,\bR)$ 
