...
\[
\begin{split}
(\omega(\exp(w,t))\phi)(w') =& e^{\pi \lambda \inn{w'}{Jw'}/2} f(\exp(w') \exp(w,t))\\
=& e^{\pi \lambda \inn{w'}{Jw'}/2} f(\exp(w'+w,t+ \frac{1}{2}\inn{w'}{w}))\\
=& e^{\pi \lambda \inn{w'}{Jw'}/2} \chi(t+ \frac{1}{2}\inn{w'}{w}) f(\exp(w'+w))\\
=& \chi(t) e^{\frac{1}{2}\pi \lambda (\inn{w'}{Jw'}+2i \inn{w'}{w}}  f(\exp(w+w')) \\
=& \chi(t) e^{\frac{1}{2}\pi \lambda (\inn{w'}{Jw'}+2i \inn{w'}{w}} 
e^{-\pi \lambda H_J(w'+w,w'+w)/2} \phi(w+w')\\
=& \chi(t) e^{\frac{1}{2}\pi \lambda (H_J(w',w')+2i \inn{w'}{w}-H_J(w+w',w+w')} 
 \phi(w+w')\\
=& \chi(t) e^{\frac{1}{2}\pi \lambda (H_J(w',w')+2i \inn{w'}{w}
  -(H_J(w,w)+H_J(w,w')+ H_J(w',w)+H_J(w',w'))} 
 \phi(w+w')\\
=& \chi(t) e^{\frac{1}{2}\pi \lambda (-H_J(w,w)+2i \inn{w'}{w}-2\inn{w}{Jw'})} 
 \phi(w+w')\\
=& \chi(t) e^{-\pi \lambda (H_J(w,w)/2+ H_J(w,w'))} 
 \phi(w+w')
\end{split}
\]
...
\[
\omega(X) \phi = \theta_X(\phi) -\pi\lambda H_X \phi
\]
Where $H_X(w') = H(X,w')$ is a linear function.

Take $Y = \frac{1}{2}(X-iJX)$. Then $X = Y + \overline{Y}$, 
and 
\begin{align*}
\omega(Y) \phi &= \theta_X \phi \\
\omega(\overline{Y}) \phi &= -\pi \lambda H_X \phi
\end{align*}
