...
We can use 
\begin{align*}
Z_k  &= P_k-iQ_k = e_k + i e_{n+k}\\
\bZ_k &=  e_k - i e_{n+k}
\end{align*}
be the basis of $W_{\bC}$. 

Now compute the action of $\fhh$ on the oscillator representation. 
In fact 
\[
\begin{split}
e_k &= \frac{1}{2}(Z_k + \bZ_k) = \frac{1}{2}[(P_k -iQ_k) + (P_k + iQ_k)] 
=  -\theta_{z_j}  + \pi \lambda z_j\\
e_{n+k} & = i\frac{1}{2}[(P_k-iQ_k) - (P_k + iQ_k)]
= -i\theta_{z_j} -i \pi \lambda z_j
\end{split}
\]
...
\[
\cmm{Z_K}{\bZ_j} = \cmm{-2\theta_{z_k}}{2\pi \lambda z_j} = \delta_{}
\]
\[
\cmm{e_k}{e_{n+k}}= 
\]
