...
\]
Note that $S^r(\fgg)^K\subset S^r(\fgg)^{K^c}$ in our case, the above lemma
...
$Z(\fgg)$ action on $V$ and the annilater ideal $\cjj\subset \cuu(\fkk)$ of 
$\rho$.
\end{lemma}
\proof
...
$x-z \in \cjj\cuu(\fgg)$.
Note that for any $v\in V$ we have $jv \in \cnn_{V,W}$ where $j\in \cjj$.
...
$\omega(x) = \omega(x')$ and this choice is independent of the realization 
...
Let $\Gamma = \Gamma_{\fkk,M}^{\fkk,\tK}$, where $M$ is 
the maximal compact subgroup of $K$.
\begin{lemma}\label{lemma:derugkact}
Let $V$ be a $(\fgg,M)$-module, 
For $x\in \cuu(\fgg)^K$, it act on 
$\Gamma^iV$ by $\Gamma^i x$, where view $x$ as a $(\fkk,M)$ endomorphism
of $V$. 
\end{lemma}
\proof Form the realization of derive functor. \qed

\begin{lemma}\label{lemma:derugkact2}
In above setting, assume $\Gamma^iV \to \Gamma^i x$ is non-zero,
Then $\Gamma^iV$ has a composition component with $\cuu(\fgg)^\tK$
...
\efig
\]
Take any $\tK$ component $\tau$ of the image of $\Gamma^iV$, 
So $\Gamma^i(V)(\tau)$ has a $\cuu(\fgg)^\tK\times \tK$ composition component
with $c_{V,W}\otimes \tau$ action.
By the equivalent of category of $\cuu(\fgg)^\tK\times \tK$
...
there is a unique $v\in S$ such that $(e_j-if_j)\cdot v = 0$. 
The $W$ invariant subspace generated by $v$
is a model of oscillator representation of $\sp(W_0)$.

Let's look at a example. 
Let $X = \Mat_{m,n}= V\otimes W$ be the set of $m$ by $n$ matrices.
$W = X \oplus X^* = \Mat_{m,2n}$. The symplectic form on $W$ is given by 
\[
\inn{A}{B} = \tr AJB^T, \forall A,B\in W,
\]
where 
\[J = \begin{pmatrix}
  0 & I_n\\
...
$\fgg=\fso(m,\bC)$ be the Lie subalgebra of $\sp(W)$ act on left of $W$
and $\fgg'=\sp(2n,\bC)$ act on $W$ by right action. $\fmm\cap \fgg' \cong \fgl(n,\bC)$.
