...
We implement this algorithm for $d=25$, $b=4$, $\mu=0$, $\sigma=1$, $T=1$, $X_0=0$ and generate $L=350000$ replicates. The results are shown in Figure \ref{f:ab}. Since we are dealing with $\mu = 0$ currently, by the symmetric effect of a Brownian motion without drift, we only examine $\alpha$-quantiles where $\alpha$ vary from $[50\%,100\%]$. %It is well known that the convergence rate of Euler scheme for Brownian motion is $1$. 
...
\begin{equation}
\frac{\log E|\varepsilon_{2^s}|}{\log 2} = \frac{C}{\log 2} -\gamma s
\end{equation}
...
To analyze the influence of drift $\mu$ on the strong order of convergence, we have done this simulation for parameters as $\mu$, $d=25$, $b=4$, $\mu=0$, $\sigma=1$, $T=1$, $X_0=0$, $L=35000$. Results are presented in Figure \ref{f:ab3}, \ref{f:lab3} and \ref{f:ratio3}. 

% mu=0
\begin{figure}[p]
   %\centering
   \includegraphics[scale=0.8]{nout_7_27a.pdf}
   \caption{Expectation of the absolute discretization error for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($4\le s \le 16$), $\mu=0$, $\sigma=1$, $T=1$, $X_0=0$. }
\label{f:ab}
\end{figure}
 ...
 \begin{figure}[p]
   ...
   \caption{Logarithm of the absolute discretization error for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($4\le s \le 16$), $\mu=0$, $\sigma=1$, $T=1$, $X_0=0$, $L=35000$.} 
   \label{f:lab}
\end{figure}
 ...
 \begin{figure}[p]
   ...
   \caption{The strong order of convergence for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($4\le s \le 16$), $\mu=0$, $\sigma=1$, $T=1$, $X_0=0$, $L=35000$.}
   \label{f:ratio}
\end{figure}
...
\begin{figure}[p]
   ...
   \caption{Expectation of the absolute discretization error for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($4\le s \le 16$), $\mu=3$, $\sigma=1$, $T=1$, $X_0=0$, $L=35000$. }
\label{f:ab3}
\end{figure}
 ...
 \begin{figure}[p]
   ...
   \caption{Logarithm of the absolute discretization error for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($4\le s \le 16$), $\mu=3$, $\sigma=1$, $T=1$, $X_0=0$, $L=35000$.} 
   \label{f:lab3}
\end{figure}
 ...
 \begin{figure}[p]
   ...
   \caption{The strong order of convergence for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($4\le s \le 16$), $\mu=3$, $\sigma=1$, $T=1$, $X_0=0$, $L=35000$.}
   \label{f:ratio3}
\end{figure}

Although approximated strong order of convergence for $\mu=0$ and $\mu=3$ are not the same, the influence of $\mu$ is not significant. 
%sim=6455 mu=0.4
%\begin{figure}[p]
   %\centering
...
In this section we provide a theoretical analysis about the expectation of the discretization error of the Euler approximation for $\alpha$-quantiles. We find that the leading order of the expectation of the discretization error $E\varepsilon_N = E (M(\alpha,T)) - \E (M(k,N)$ is $1$ for $0 < \alpha<1/2$ or $0<\alpha<1/2$. That it the expectation of the dissertation error is $O(1/N^{-1})$ when $0 < \alpha<1/2$ or $0<\alpha<1/2$. 
...
as $\widetilde c(\mu,\alpha)$. In Figure \ref{}, we can see the influence of $\mu$ and $\alpha$ on the coefficient of the leading term $c(\mu,\alpha)$. Fixed $\mu$, we find that the closer $\alpha$ to $1/2$, the closer $c(\mu,\alpha)$ to $0$. Fixed $\alpha$, we find that the larger the value of $\mu$, the closer $c(\mu,\alpha)$ to $0$.

Since the discretization error for the maximum is axisymmetric with respect to $\mu=0$, we find that 
...
\[
Err_{j,s} = \frac{1}{L}\sum_{1\leq l \leq L} {M_{j,d}^{(l)}- M_{j,s}^{(l)}}.
\]

We realize this algorithm for $d=25$, $b=4$, $\mu = 0$, $\sigma^2=1$, $T=1$, $X_0=0$ and generate more than 2000 sampled paths. As you can see in Figure \ref{f:err}, Figure \ref{f:lerr} and Figure \ref{f:rate}, the simulation results are coincident with our theoretical conclusion. 
