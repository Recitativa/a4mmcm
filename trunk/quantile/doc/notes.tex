...
Absence of arbitrage is the fundamental principle of option pricing. \citet*{Black:1973} is the landmark paper in the field of option pricing. \citet*{merton1973} was the first to extend this valuation model to incorporate dividend payments. In his paper, the rationality of option pricing was also discussed. 
...
\begin{enumerate}[1.]
\item Investors are able to borrow and lend cash at the same risk-free interest rate $r$. This risk-free interest rate is supposed to be known and constant. 
...
Let $V(S, t)$ be the price of a European vanilla option at time $t$ when stock price is $S$. The value $V(S,T)$ at maturity $T$ is the payoff. To get the value of a option at an earlier time, we should know how its price changes as a function of stock price $S$ and time $t$. Suppose $V(S,t)$ is twice differentiable, applying It\^o's formula (\cite{Steven}~(4.4.24)) to $V$ we get
...
More precisely, a stochastic process $X_t$ is called It\^o process (\cite{Steven}~Definition~4.4.3) if
\[
...
For any function $f(t,x)$ with continuous partial derivatives 
$\partial{f}/\partial t, \partial f/\partial x$ and $\partial^2 f/\partial x^2$,
 the It\^o's formula for It\^o process is given by
\[
d f(t,X_t) = \frac{\partial f}{\partial t}(t,X_t) dt +\frac{\partial f}{\partial x}(t,X_t) dX_t
+ \frac{1}{2} \frac{\partial^2 f}{\partial x^2}(t,X_t) \sigma_t^2 dt
\]
Note that the stochastic process $S_t$ is a It\^o process. Applying above result we get equation~(\ref{ito}).
...
Let $f(S)$ be the payoff function of an option, then the boundary conditions for this option are
...
The sharp advantage of the Monte Carlo method is that it is convenience to calculate high dimensional integrations. 
...
The definitions of strong order and weak order of convergence of a numerical solution to stochastic differential equation also can be found in \cite{lacus2008} and \cite{Peter1999}. This idea of using order to measure the convergence speed of numerical solution is inherited from the similar convention used to measure the convergence speed of numerical solution to deterministic differential equation. We notice that, mathematically, for a numerical algorithm the $\gamma$ is not unique by this definition, since $O(h^{\gamma_1})$ is $O(h^{\gamma_2})$ if $\gamma_1 > \gamma_2$. But since $\gamma$ indicates the speed of convergence, it has a practical meaning. Conventionally, when we talk about order, we mean the largest $\gamma$.
