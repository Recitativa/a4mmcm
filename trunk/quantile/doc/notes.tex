...
Using the same algorithm in section Euler scheme and random walk, we ran a simulation to examine the above claim. 
...
\item Calculate the quantiles for sequence \{$X_j$, $j=1, 2, \cdots, 2^{27}$\}, use these quantiles as the continuous quantiles $M(\alpha , T)$
\item Calculate the quantiles for sequence \{$X_j$, $j=k2^s$\}, $k=1, \cdots, 2^{27-s}$
\item repeat the above for 1365 times and calculate the errors
\end{enumerate}
The result: 
\begin{figure}[htbp]
   \centering
