...
As we expected, the convergence ratio of maximal is around $1/2$.
Moreover, for other quantiles, the strong order of convergence is far away from $1/2$, all of them are around to $0.75$.
These simulations reflected the huge difference of strong order of convergence for genuine quantiles and maximal.
...
as $\widetilde c(\mu,\alpha)$. In Figure \ref{plot-mu}, we can see the influence of $\mu$ and $\alpha$ on the coefficient of the leading term $\widetilde c(\mu,\alpha)$. Fixed $\mu$, we find that the closer $\alpha$ to $1/2$, the closer $c(\mu,\alpha)$ to $0$. Fixed $\alpha$, we find that the larger the value of $\mu$, the closer $\widetilde c(\mu,\alpha)$ to $0$.
...
Further more, we provide the comparison between the theoretical values and simulation results of $log(C(\mu,\alpha))/log(2)$ in Table~\ref{t:int0} and Table~\ref{t:int3}. We can see that they matched pretty well. 
...
As we can seen, $\widetilde c(\mu,\alpha)$ decreased dramatically when $\alpha$ closing $0.5$ or $\mu$ increasing.  
...
\begin{figure}[p]
   ...
   \caption{Logarithm of the discretization error for selected $\alpha$-quantile ($\alpha = 0.5625, 0.625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($4\le s \le 16$), $\mu=0$, $\sigma=1$, $T=1$, $X_0=0$, $L=500000$}
   \label{f:lerr}
\end{figure}
