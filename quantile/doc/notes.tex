...
\hline
\cite{Boyle1986} & $x_1 = l\sigma\sqrt{h}$ & $p_1 = \frac{1}{2l^2} + \frac{(r-\sigma^2 /2)\sqrt{h}}{2l\sigma}$ \\
 & $x_2 = 0$ & $p_2 = 1-\frac{1}{l^2}$ \\
 & $x_3 = -l\sigma\sqrt{h}$ & $p_3 = \frac{1}{2l^2} - \frac{(r-\sigma^2 /2)\sqrt{h}}{2l\sigma}$\\
\hline
...
In the first step to get the Euler approximation of the $\alpha$-quantile of a Brownian motion at time $T$, we divide the time interval $[0,T]$ into $N$ equal small time intervals. Then the Euler approximation for the $\alpha$-quantile $M(\alpha, T)$ of the Brownian motion $\{X_t\}_{t\leq T}$ is the $k$-th order statistic of the set $\{X_{nh},n=0,1,\cdots,N\}$ with $k= \lfloor \alpha  N\rfloor$. Since $h$ and $N$ are related by $h=T/N$, we denote the Euler approximation of $M(\alpha, T)$ with $h=T/N$ as $M(k,N)$. Then the discretization error for the Euler approximation of the $\alpha$-quantile of a Brownian motion at time $T$ is 
\begin{equation}
...
We implement this algorithm for $d=27$, $b=6$, $\mu=0$, $\sigma=1$, $T=1$, $X_0=0$ and generate $L=6455$ replicates. The results are shown in Figure \ref{f:ab}. Since we are dealing with $\mu = 0$ currently, by the symmetric effect of a Brownian motion without drift, we only examine $\alpha$-quantiles where $\alpha$ vary from $[50\%,100\%]$. %It is well known that the convergence rate of Euler scheme for Brownian motion is $1$. 
...
Suppose the expectation of the absolute discretization error is $O(1/N^\lambda)$, then we get
\begin{equation}\label{error-N-sim}
E|\varepsilon_N| \sim C/N^\lambda,
\end{equation}
where $C$ is some constant. Considering in our simulation $N$ is expressed as $N = 2^s$, take logarithm to both sides of (\ref{error-N-sim}) and with easy math we get
\begin{equation}
\frac{\log E|\varepsilon_{2^s}|}{\log 2} = \frac{C}{\log 2} -\lambda s
\end{equation}

The similarity of the order of convergence for $\alpha$-quantiles when $\alpha$ is between $[50\%, 100\%)$ is more obvious after we take the logarithm. In Figure \ref{f:lab}, you can see that except for the maximum, i.e., the $100\%$-quantile, the strong order of  convergence for other $\alpha$-quantiles of Brownian motion are close to each other. This is expressed as the lines parallel to each other. Figure \ref{f:ratio} shows the strong order of convergence for different $\alpha$-quantiles. You can see that the strong order of convergence for $100\%$-quantile is close to ${1/2}$, while strong order of convergence for other quantile are around $0.76$.  

%sim=6455 mu=0
\begin{figure}[p]
   %\centering
   \includegraphics[scale=0.8]{nout_7_27a.pdf}
   \caption{Expectation of the absolute discretization error for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($7\le s \le 18$). }
\label{f:ab}
\end{figure}
 ...
 \begin{figure}[p]
   ...
   \caption{Logarithm of the absolute discretization error for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($7\le s \le 18$)} 
   \label{f:lab}
\end{figure}
 ...
 \begin{figure}[p]
   ...
   \caption{The strong order of convergence for selected $\alpha$-quantiles ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($7\le s \le 18$)}
   \label{f:ratio}
\end{figure}
...
In this section we provide a theoretical analysis about the expectation of the discretization error of the Euler approximation for $\alpha$-quantiles. We find that the leading order of the expectation of the discretization error $E\varepsilon_N = E (M_{\alpha,T}) - \E (M_{k,N})$ is $1$ for $0 < \alpha < 1$. That it the expectation of the dissertation error is $O(1/N^{-1}$ when $0 < \alpha <1$. 
...
=& -\frac{\zeta(1/2)}{\sqrt{2\pi}}\sigma \sqrt{\frac{\alpha T}{\alpha N}} - \frac{2g(\mu_1)-\mu_1}{4N}\sigma\sqrt{\alpha T} +O(1/N^{{3}/{2}}) \\
&-\left(\frac{\zeta(1/2)}{\sqrt{2\pi}}\sigma \sqrt{\frac{(1-\alpha)T}{(1-\alpha)N}} - \frac{2g(\mu_2) - \mu_2}{4N}\sigma \sqrt{(1-\alpha)T} + O(1/N^{{3}/{2}})\right) \\
=& -\frac{\zeta(1/2) \sigma}{\sqrt{2\pi}}\left(\sqrt{T/\alpha N}
\sqrt{\alpha} - \sqrt{T/(1-\alpha)N}\sqrt{1-\alpha}\right) \\
& - \frac{\sigma\sqrt{T}}{4N}\left((2g(\mu_1)-\mu_1)\sqrt\alpha - (2g(\mu_2)-\mu_2)\sqrt{(1-\alpha)}\right) \\
&+ O(1/N^{{3}/{2}}) \\
=&- \frac{\sigma\sqrt{T}}{4N}\left((2g(\mu_1)-\mu_1)\sqrt\alpha - (2g(\mu_2)-\mu_2)\sqrt{(1-\alpha)}\right) \\
& + O(1/N^{{3}/{2}}),
\end{align*}
where $k = \alpha N$, $\mu_1 =\mu\sqrt{\alpha T}/\sigma$, and $\mu_2 =\mu\sqrt{(1-\alpha)T}/\sigma$.
The third last equality holds by view $\displaystyle\inf_{t\leq (1-\alpha)T} S'_t$ as
...
Using the same algorithm in section \ref{strong-order-convergence}, we have done a simulation to examine the above claim. 
Here we restate the parameter setting and algorithm.
...
\item Repeat the above for $L$ times,
  get replicates $\Set{M_{j,s}^{(l)},{1\leq l\leq L}}$.
\item Approximate the expectation of the discretization error for $s < d$ using:
\begin{align}
...
\begin{figure}[p]
   ...
   \caption{Expectation of the discretization error for selected $\alpha$-quantile ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($7\le s \le 18$) }
   \label{f:err}
\end{figure}
...
\begin{figure}[p]
   ...
   \caption{Logarithm of the discretization error for selected $\alpha$-quantile ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($7\le s \le 18$)}
   \label{f:lerr}
\end{figure}
...
\begin{figure}[p]
   ...
   \caption{The order of convergence for selected $\alpha$-quantile ($\alpha = 0.5, 0.5625, \cdots, 1$) using the Euler scheme with $N = 2^s$ ($7\le s \le 18$)}
   \label{f:rate}
\end{figure}
...
$\alpha$-quantile options, also known as quantile options, are a type of options whose payoff are functions of the quantiles of underlying asset price process. Firstly proposed in \cite{Miura}, the payoff function at maturity for a European-style $\alpha$-quantile call option with strike price $K$ and stock price at innitiation $S_0$ is 
\begin{equation}
(S_0e^{M(\alpha,T)}-K)^+,
\end{equation}
...
Discrete the time, i.e. let $0=t_0< t_1, \cdots, t_N =T$. Let $P_k=(X_0, \cdots, X_k)$ be a path in a binomial tree approximating the movements of a Geometric Brownian motion till time ${t_k}$. 
...
In this thesis, we provide a numerical study about the strong order of convergence of the Euler approximation associated with the simulation of the $\alpha$-quantile of Brownian motion. We find that the strong order of convergence for genuine $\alpha$-quantile of Brownian motion where $\alpha \in (0,1)$ is around $0.76$, while the strong order of convergence for the maximum or the minimum of Brownian motion is $0.5$. We provide an estimate about the expectation of  the discretization error for the Euler approximation of the $\alpha$-quantile of Brownian motion. This estimate can be used to connect the price of continuously and discretely monitored $\alpha$-quantile options in future work. 
