...
As $V_t$ and $S_t$ share the same underlying, the source of uncertainty is the same for both. Thus we can eliminate Brownian motion $B_t$ by constructing a certain portfolio. The appropriate trading strategy is holding one option and $-{\partial V}/{\partial S}$ shares of stock.
...
\begin{cases}
\displaystyle V(S_T,T) =f(S_T)  & \text{on } \bR_+ \\
\displaystyle V(0,t)=f(0)e^{-r(T-t)} & \text{on } [0,T) \\
\displaystyle\lim_{S \to \infty} V(S,t)= f(\infty)e^{-r(T-t)} & \text{on } [0,T).
\end{cases}
\end{equation}

The second equation in (\ref{Black-boundary}) means that if stock price $S_t$ is $0$ all the time in its life, then the price of the option at any time should be the discounted pay off at risk-free rate. The last equation in (\ref{Black-boundary}) is obtained under the same logic. 

PDE (\ref{black-pde}) is the fundamental valuation equation for the standard normal option. More over, this PDE applies to both European and  American plain vanilla options, regardless payoff functions. For different options, what should change is equation (\ref{Black-boundary}), i.e., boundary conditions.

There are many transformations that can convert the Black-Scholes PDE into a heat equation. In this thesis, we present the transformation suggested in \cite{Wilmot1994}. 

Under transformation
\begin{equation}
\begin{cases}
 S=Ke^x \\
\displaystyle t=T-2\tau/\sigma^2 \\
\displaystyle V(S,t)=e^{-\alpha x - \beta \tau } v(x, \tau)\\
\displaystyle \alpha=(r- \sigma ^2 / 2) / \sigma ^2 \\
\displaystyle \beta= \alpha^2 + 2r/{\sigma}^2, 
\end{cases}
\end{equation}

$v(x,\tau)$ satisfies the heat equation
\begin{equation}
...
\begin{equation}
\begin{cases}
e^{-\alpha x} v (x,0) = f(K e^x) & \text{on } \bR \\
\displaystyle \lim_{x \to -\infty}e^{-\alpha x- \beta \tau } v(x,\tau) = f(0)e^{-2r\tau / \sigma ^2} &\text{on } \tau \in [0,\sigma ^2 T /2] \\
\displaystyle \lim_{x \to \infty} e^{-\alpha x -\beta \tau } v(x,\tau) = f(\infty) e^{-2r\tau / \sigma ^2} & \text{on } \tau \in [0, \sigma ^2 T /2].
\end{cases}
\end{equation}

The heat equation was originally used to describe the distribution of heat in a given medium as time passes. It is a well known subject in Physics and has been thoroughly studied. If $\lim_{x\to \pm \infty} v(x, \tau)= 0$, the normal density function $v(x,t) = (2 \sqrt{\pi t} )^{-1} \exp(-x^2 /{4t})$ with mean $0$ and standard deviation $\sqrt{2t}$ is the fundamental solution.
...
c(S_t,t;K)=S_t \Phi(d(S_t;K,T-t))-Ke^{-r(T-t)} \Phi(d(S_t;K,T-t)-\sigma\sqrt{T-t}),
\end{equation}
...
\end{equation}
Equation (\ref{blfc}) is known as the Black-Scholes formula for a European-style call option with strike $K$ and maturity date $T$.

Similarly, the pricing formula for a European-style put option is
\begin{equation}\label{blfp}
p(S_t, t;K)= Ke^{-r(T-t)} \Phi(-d(S_t ; K,T-t) +\sigma \sqrt{T-t})-S_t \Phi(-d(S_t;K,T-t)).
\end{equation}
...
As revealed in the Black-Scholes PDE (\ref{black-pde}) and the corresponding boundary conditions (\ref{Black-boundary}), the market parameters that are relevant to the price of options include interest rate $r$, volatility $\sigma$, strike price $K$, stock price $S_0$ and time to maturity $T-t$. The expected stock return rate $\mu$, very surprisingly, does not have any influence on the option price. The Black-Scholes PDE also does not involve any parameter regarding the risk preference of investors. This phenomenon brings a hint to another approach of option valuation, risk-neutral valuation. 
...
However, with the knowledge of $S$, $u$ and $d$, can we get the price of the asset in a different order? That is, can we find a new probability pair $p$ and $1-p$ that can substitute $q$ and $1-q$ in the calculation of asset price which allows us to use the risk-free interest rate $r$ instead of risky discount factor $k$? If $d < 1+r < u$, that is, in no-arbitrage world, the answer is affirmative. 
...
\end{equation}
Under the equivalent martingale measure $\mathbb P$, the price of a European option can be stated as 
\begin{equation}
V(S_t,t)=e^{-r(T-t)}\mathbb E[f(S_T)|\mathcal{F}_t],
\end{equation}
where $\mathcal {F}_t$ is the natural filtration corresponding to the Brownian motion $\{B_t\}$. Readers can refer to \cite{Bingham1998} to get a deep understanding. 
%Interesting study about this model can be found in \cite{HarrisonKreps1979}, \cite{Harrison1981} and \cite{Kreps1981}.

\section{American options}
In the risk-neutral valuation model, the price the American options can be defined as a solution to an optimal stopping time problem. 

At time $t$, the price of an American option can be given by
\begin{equation}\label{Am-rn-equation}
V(S,t) =\sup_{t\leq \tau\leq  T}\mathbb E[e^{-r(\tau-t)}f(S_\tau)|\mathcal{F}_t],
\end{equation}
where $\tau$ denotes a stopping time taking values in $[t,T]$ and
\[
f(S,t) = 
\begin{cases}
...
We can understand (\ref{Am-rn-equation}) by noticing that $\tau$ corresponds to possible strategies to exercise the option. More precisely, the holder will exercise the option at time $\tau$. The American option price corresponds using the optimal strategy. It can be shown that the optimal exercise strategy is represented by some exercise region $D$ in $(S,t)$-plane, i.e. $\tau<t$ if and only if $(S_t,t)\in D$. This domain $D$ is called as exercise region. 
...
\begin{equation}
\begin{split}\displaystyle
V(S_{\max},t;K) &= S_t \Phi(d) - Ke^{-r(T-t)}\Phi(d-\sigma \sqrt{T-t}) \\
...
\begin{equation}
\begin{split}
V(S_{\max},t;K) = & e^{-r(T-t)}(S_{\max}^t - K) + S_t \Phi(d') - S_{\min}^t e^{-r(T-t)} \Phi(d'-\sigma \sqrt{T-t}) \\
...
\begin{equation}
\begin{split}
V(S_{\min},t;K)=& -S_t \Phi(-d) + Ke^{-r(T-t)} \Phi(-d + \sigma \sqrt{T-t})  \\
...
\begin{equation}
\begin{split}
V(S_{\min},t;K) = & e^{-r(T-t)} (K-S_{\min}^t) - S_t \Phi(-d'') + S_{\min}^t e^{-r(T-t)} N (-d'' + \sigma \sqrt{T-t}) \\
...
For the American-style lookback option, one should provide both the option price and the optimal exercise strategy to completely solve the pricing problem. Due to this complexity, usually we need to use a numerical approach to solve the problem of pricing American-style lookback options. In \cite{Lim2004}, they provide a complete solution to compute the price and the optimal exercise boundary. In that paper, they proposed a space-time transformation which can help the pricing of options of this type Under this transformation, calendar time is scaled by the square of volatility $\sigma^2$. Therefore, the canonical time horizon is much smaller than time to expiration $T-t$. In addition, the dependence on a prescribed root node which then makes computing the entire early excise boundary by backward induction possible.
...
There are plenty of proposals for tree methods. The popularly used are \cite{cox1979},\cite{jarrow1983},\cite{Boyle1986} and \cite{Amin1991}. We present them in Table \ref{well-know-lattic-methods}.
...
\end{equation}
is one popular measure of the quality of estimation and here $\|\;\| _2$ is the Euclidean distance. If (\ref{strong}) is $O(\Delta t^\gamma)$ as $\Delta t \downarrow 0$, this approximation is claimed to {\it converge strongly} with order $\gamma > 0 $. Strong convergency is important when fine path-wise approximation is need. Meanwhile, an approximation {\it converges} weakly with order $\beta > 0$ if, for any function continuous and $g: \bR \to \bR$ such that all derivatives up to and including $\lceil 2(\beta + 1) \rceil$ exsit and have polynomial growth, 
\begin{equation}
