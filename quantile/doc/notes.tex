...
If we get some results by binomail tree of European Option.
With 
\[
...
$S_0$ & $90$ & $95$ & $100$ & $105$ & $110$ & $115$ & $120$\\
\hline
  30 & 1.80059 & 3.46444 & 5.98097 & 9.35544 & 13.3482 & 17.7490 & 22.3748 \\
  32 & 1.78771 & 3.44615 & 5.95781 & 9.32707 & 13.3219 & 17.7186 & 22.3409 \\
  34 & 1.77631 & 3.43422 & 5.93868 & 9.30442 & 13.2968 & 17.6906 & 22.3096 \\
  36 & 1.76377 & 3.42090 & 5.92051 & 9.28269 & 13.2762 & 17.6641 & 22.2809 \\
  38 & 1.75524 & 3.41137 & 5.90549 & 9.26586 & 13.2571 & 17.6396 & 22.2588 \\
 Paper & 1.6588 & 3.2695 & 5.7501 & 9.0895 & 13.0213 & 17.4030 & 22.0456\\
\end{tabular}

Data is from \cite{Laura2001}
...
\subsection{connecting the discrete and continous $\alpha$-quantile options}
...
&+E(e^{M_m}-x;e^{M_m}\geq k > e^M)
-E(e^M-x;e^M\geq k > e^{M_m})\\
=& A - B + C + D
\end{split}
\]
...
Then, 
\[
0 \leq C \leq E(e^{M_m}-e^M;e^{M_m}\geq k > e^M)
\] 
Since $P(e^{M_m}\geq k > e^M) \rightarrow 0$,
\[
E(m(e^{M_m}-e^M);e^{M_m}\geq k > e^M) \rightarrow 0
\]
by the uniformly integrability. So $C$ and similary $D$ is $o(1/m)$.
If the assumption is not true, if we can prove
\begin{equation}
E(e^{M_m}-e^M;e^{M_m}\geq k > e^M) \sim o(1/m) \label{eq:sp1}
\end{equation}
it is also OK. 
...
=& e^{\beta/m}E(e^M-xe^{-\beta/m};e^M>xe^{-\beta/m})  \notag\\
 & +e^{\beta/m}E(e^M-xe^{-\beta/m};e^M\in [xe^{-\beta/m}, x]) \label{eq:sp3} 
o(1/m)\\
= &  e^{\beta/m}E(e^M-xe^{-\beta/m};e^M>xe^{-\beta/m} + o(1/m) \label{eq:cor2}
\end{align}
...
Now we disscuse the equation (\ref{eq:sp2}).
Use Taylor expension, we get
\[
e^M-e^{M_m} = e^M(M-M_m) + \frac{1}{2}e^{\bM_m}(M-M_m)^2
\]
For some $\bM_m$ between $M$ and $M_m$.
Now assume 
\[
m^\gamma(M-M_m) \darrow W
\]


Now both equation (\ref{eq:cor1}) and (\ref{eq:cor2}) 
are correction formulars.
