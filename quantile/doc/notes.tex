...
\subsection{Euler scheme and random walk}
...
\cite{Janssen2008} gives a estimate of the difference of  expectations
...
&-\left(\frac{\zeta(1/2)}{\sqrt{2\pi}}\tsigma \sqrt{\frac{(1-\alpha)T}{(1-\alpha)N}} - \frac{2g'(\mu_2) - \mu_2}{4N}\tsigma \sqrt{(1-\alpha)T} + O(1/N^\frac{3}{2})\right) \\
...
Using the same algorithm in section Euler scheme and random walk,
...
\begin{align}
Err_{j,s} &= \abs{\frac{1}{L}\sum_{1\leq l \leq L} M_{j,s}^{(l)}- M_{j,d}^{(l)}}
\end{align}
\end{enumerate}
...
Now, suppose that $f$ is an monotone Lipschitz continuos function.
WLOG, asume 
