...
From this moment on, innovation has bred the creation of copious products tailored to fill the needs of different types of investors. Path-Dependent options, such as Asian options, barrier options and lookbacks are traded in market routinely.
...
Let $V(S, t)$ be the value of an option at time $t$. The at maturity $T$, $V(S,T)$ is the payoff. To get its value at an earlier time, we should know how it changes as a function of $S$ and $t$. Suppose $V(S,t)$ is twice differential, applying It$\breve{o}$'s lemma to $V$ we get:
...
Lookback option is a type of exotic options whose price have path dependency effects. 
...
\hline
lookback & 5.876317 &  12.97765 & 19.16763 & 24.43289
\end{tabular}
\end{center}
\label{fig:euro5}
\end{table}%
...
lookback & 8.381566 & 12.81988 & 16.06060
\end{tabular}
\end{center}
\label{fig:euro8}
\end{table}%
...
K=100, r=0.05, \sigma=0.2, \alpha=0.5, T=1
\]
, with our tree method, we get the price of American $\alpha$-quantile option in Table \ref{fig:amer5}. Comparing data in Table \ref{fig:euro5} and Table \ref{fig:amer5}, we can see price for American option is more expensive than European option with same feature. This again insure that our method is correct. 
